	...
	public void render(Graphics g) {
		if (!isDamaged) {
			if (dir == right_dir) {
				...
				if (hasGun) {
					g.drawImage(Entity.cane_en_right, this.getX() - Camera.x + 10, this.getY() - Camera.y + 7, null);
				}
			} else if (dir == left_dir) {
				g.drawImage(leftPlayer[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
				if (hasGun) {
					g.drawImage(Entity.cane_en_left, this.getX() - Camera.x - 6, this.getY() - Camera.y + 9, null);
				}
			}
			if (dir == up_dir) {
				...
				if (hasGun) {
					g.drawImage(Entity.cane_en_down, this.getX() - Camera.x + 7, this.getY() - Camera.y + 10, null);
				}
			}
		} else {
